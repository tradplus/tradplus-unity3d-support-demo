...
public class TradPlusAndroidNative
{
    ...
    //加载广告，设置位置（底部中间、顶部中间）
    public void CreateNative(TradPlus.AdPosition position)
    {
        _nativePlugin.Call("createNative", (int)position);
    }

    //加载广告 设置坐标参数
    //两种加载广告的方法只能二选一
    public void CreateNative(int x, int y)
    {
        _nativePlugin.Call("createNative", x , y);
    }
    ...
    //进入广告场景
    public void ShowNativeEntryAdScenario()
    {
        _nativePlugin.Call("entryAdScenario");
    }
}
