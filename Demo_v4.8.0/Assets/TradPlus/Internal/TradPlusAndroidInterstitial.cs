...
public class TradPlusAndroidInterstitial
{
    ...
    //进入广告位所在界面时调用
    public void ShowInterstitialEntryAdScenario()
    {
        _interstitialPlugin.Call("entryAdScenario");
    }

    //进入广告位所在界面时调用（广告场景ID）
    public void ShowInterstitialEntryAdScenario(string adSceneId)
    {
    ...
    //Check广告缓存数是否已达配置上限
    public bool IsInterstitialAllReady {
        get { return _interstitialPlugin.Call<bool>("isAllReady"); }
    }
